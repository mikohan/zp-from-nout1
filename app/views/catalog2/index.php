...
<?php include ANG_ROOT . "public/tpl/header1.php"?>
        ...
        <meta name="description" content="<?=title($data2[0]['name'])?> <em>Задняя ось</em> для Хендай Портер в наличии. Подбор по каталогу за 5 минут. На 60% дешевле, чем у официалов! ☎ <?=TELEPHONE1?> <?=title($data2[0]['id'])?>">
                ...
                <div class="col-lg-9 col-md-9 col-sm-9">
                    <div class="ang-back">
                        <ul class="breadcrumb">
                            ...
                            <li itemscope itemtype="http://data-vocabulary.org/Breadcrumb">
                                <a href="<?=ANG_HTTP?>/category2/<?=title($data2[0]['id_h2'])?>/" itemprop="url"><span itemprop="title"><?=title($data4[0]['name'])?></span></a>
                             </li>
                    ...
                    <p  class="thumbnail ang-name" data-toggle="tooltip" data-placement="top" data-original-title="Кликни по синему номеру на картинке">
                      <img id="A" usemap="#map" src="/public/img/p2<?= $data[0]['img']?>"  alt="<?= $data2[0]['name']?> Хендай Портер" title="<?= $data2[0]['name']?> Хендай Портер" ></p>
                    <?php  //print_r($data);?>
                            <map name="map"  id="map">
                                <?php foreach($data3 as $line) {
                                    ...
                                    if ($line['1c_id']!=$_POST['article']) {
                                      ?>
                                        <area id="<?=$line['id']?>" class="blue"  shape="rect" coords="<?=$line['coords']?>" href="<?php echo 'https://' .  $_SERVER['HTTP_HOST'] . '/part/'. $line['1c_id'] . '/' . str2url($line['ang_name']);?>/" alt="<?=trim($line['ang_name']) . ' Хендай Портер'?>" >
                                    <?php }else{
                                    ...
                                    ?>
                                    <area id="<?=$line['id']?>" shape="poly" coords="<?=$line['coords']?>" href="<?php echo 'https://' .  $_SERVER['HTTP_HOST'] . '/part/'. $line['1c_id'] . '/' . str2url($line['ang_name']);?>/" alt="<?=trim($line['ang_name']) . ' Хендай Портер'?>" >
                 ...
                 <?php  foreach ($data3 as $key => $line2) {
                           if($line2['1c_id'] == null OR $line2['cat'] == FALSE){
                           continue;
                           }
